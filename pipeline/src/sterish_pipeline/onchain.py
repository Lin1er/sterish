...
# Typed contract errors. The numbers are part of each frozen ABI — never renumber.
# They are per-contract and collide across contracts, so the name is only resolved
...
ESCROW_ERRORS = {
    1: "NotInitialized", 2: "RequestNotFound", 3: "NotOpen", 4: "NotBonded",
    5: "AlreadySettled", 6: "AlreadySlashed", 7: "InvalidAmount",
    8: "InvalidInput", 9: "SelfAudit",
}

# The Stellar Asset Contract is not ours, and its errors surface through whichever
# of our contracts invoked it. #10 is the one that actually bites: a transfer the
# payer cannot cover.
SAC_ERRORS = {
    10: "BalanceOutOfRange (payer cannot cover the transfer)",
}
...
class ContractCallError(OnChainError):
    ...
    Which ABI names the code is inferred from the function, so a caller does not
    have to remember, and a code the contract's own table does not claim still
